    ...
    if (_fileExt == ".wav" || _fileExt == ".ttf") {
      if (e.target.files != undefined) {
        ...
        reader.onload = function (e) {
          ...
          // wav 파일 처리하기 
          fss.writeFile(fileName2, Buffer(new Uint8Array(datas))); // 버퍼 배열로 파일 저장
        };
        fileName2 = e.target.files[0].name;
        console.log(fileName2);
    ...
    if (_fileExt == ".wav" || _fileExt == ".ttf") {
      if (e.target.files != undefined) {
        ...
        reader.onload = function (e) {
          ...
          var bId = Blockly.mainWorkspace.getBlockById(blockId)
          block.setFieldValue(JSON.stringify(fileName2), 'file_path');
        };
        fileName2 = e.target.files[0].name;
        console.log(fileName2);
        reader.readAsArrayBuffer(e.target.files[0]);  // 웨이브 파일은 버퍼로 읽어온다    
...
function localFolderOpen(blockId) {
  ...
  input.onchange = (e) => {                         // input의 값이 변한다 = 파일을 불러온다
    ...
    let block = demoWorkspace.getBlockById(blockId);                    // 블럭안의 input에 폴더 경로를 적기위해 블럭 찾음
    block.setFieldValue(JSON.stringify(directoryName), 'folder_path');  // 블럭안에 input에 폴더 경로를 적어줌
  }
}
/*
추가 구현해야 할 부분
