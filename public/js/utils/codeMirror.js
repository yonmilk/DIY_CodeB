...
function codeUpdate(event) {
	...
	try {
		var pyCode = Blockly.Python.workspaceToCode(workspace); // 워크스페이스를 코드로 변환 후 값 저장
	...
	coderun().then(() => {
	try {
		var jsCode = Blockly.JavaScript.workspaceToCode(workspace); // 워크스페이스를 js코드로 변환 후 값 저장
